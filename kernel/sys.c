...
static void get_memory_info(struct memory_info *mem_info) {
    ...
    mem_info->cached_memory = cached_pages * PAGE_SIZE;
    mem_info->swap_total = swap_total * PAGE_SIZE;
    mem_info->swap_free = swap_free * PAGE_SIZE;
    mem_info->swap_used = (swap_total - swap_free) * PAGE_SIZE;
    mem_info->buffered_memory = buffered_pages * PAGE_SIZE;
}
